...
class TestSaleLoyaltyIncompatibility(LoyaltyIncompatibilityCase):
    ...
    def test_01_program_without_incompatibilities(self):
        """When there are no incompatibilities, both coupons and
        promotions can be applied to the same sales order."""
    ...
    def test_02_program_with_incompatibilities(self):
        """When an incompatible promotion is set up, the
        incompatible promotion cannot be applied with the coupons."""
        # Create coupons for the coupon program
        self.env["loyalty.generate.wizard"].with_context(
