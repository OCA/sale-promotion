...
class SaleLoyaltyRewardProductLineWizard(models.TransientModel):
    ...
    _description = "Wizard for Managing Loyalty Reward Product Options"
    ...
    gift_options_description = fields.Text(
        compute="_compute_gift_options_description", string="Gift Options"
    )
    ...
    def _compute_gift_options_description(self):
        for record in self:
            products = ", ".join(record.gift_ids._origin.mapped("display_name"))
            record.gift_options_description = f"Choose between: {products}"
