...
group of customers with a promotion. The restriction can be defined at 2
levels:
...
only be available to customers that match the filter. When a restriction
is defined at the rule level, the rule will only be applied to customers
that match the filter. If a restriction is defined at both levels, the
program's rules will only be applied to customers that match both
filters.
