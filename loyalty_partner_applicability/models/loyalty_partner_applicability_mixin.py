...
class LoyaltyPartnerApplicabilityMixin(models.AbstractModel):
    ...
    partner_ids = fields.Many2many(
        ...
        help="""This field allow to restrict the applicability
        to a specific set of partners""",
    ...
    def _is_partner_valid(self, partner):
        ...
        if self.partner_domain and self.partner_domain != "[]":
            # (partner_valid and self.partner_ids) is required since we assume
            # that if the partner_ids is not set but the partner_domain is set,
            #  the partner must match the partner_domain (IOW before we check
            # the domain we assume that the partner is not valid if partner_ids
            # is not set and partner_domain is set)
            partner_valid = (partner_valid and bool(self.partner_ids)) or (
                partner.filtered_domain(self._get_eval_partner_domain()) == partner
            )
        # if the partner is not valid but coupon sharing is allowed,
        # check if the partner has the same commercial parent as
        # the partner in the restrictions
