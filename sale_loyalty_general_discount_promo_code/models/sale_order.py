...
class SaleOrder(models.Model):
                ...
                reward_line.update(
                    {
                        ...
                        "name": _(f"You saved {saved_amount} with promo {coupon.code}"),
                        "price_unit": 0,
                        "product_uom_qty": 0,
                        "product_id": None,
                        "product_uom": None,
    ...
    def update_discount_percentage(self):
        ...
        for line in order_lines:
            ...
            # Update discounr based on `specific` rewards
    ...
    def _try_apply_code(self, code):
        self.ensure_one()
        reward_line = self.order_line.filtered(lambda line: line.reward_id)
        if reward_line:
            raise UserError(
                _(
                    "Only one coupon code is allowed. "
                    "Please delete the previous coupon code to use the new coupon code."
                )
            )
