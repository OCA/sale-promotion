...
class TestSaleCouponChainable(TestSaleCouponCommon):
    def setUp(self):
        super(TestSaleCouponChainable, self).setUp()
    ...
    def test_program_chainable_multi_discounts(self):
        ...
        self.assertAlmostEqual(
            ...
            2,
            "Chainable promo should chain, other non chainable should apply "
            "on full total.",
        )
    ...
    def test_program_chainable_on_various_specific_products(self):
        ...
        self.assertAlmostEqual(
            ...
            2,
            "Chainable specific products discount should chain on common "
        ...
        self.assertAlmostEqual(
            ...
            2,
            "Chainable specific products discount should chain twice on "
            "common products and on global discount",
        )

    def test_program_chainable_order_min_quantity(self):
        order = self.empty_order
        self.env["sale.order.line"].create(
            {
                "product_id": self.largeCabinet.id,
        ...
        program.rule_minimum_amount_chained = True
        order.recompute_coupon_lines()
        ...
        self.assertAlmostEqual(
            ...
            347.4,  # 386 - 0.1 * 386
            2,
            "The chainable discount is not applied since the minimum amount is not reached",
        )

        program.rule_minimum_amount = 300.00
        ...
        self.assertAlmostEqual(
            ...
            330.03,  # 386 - 0.1 * 386 - 0.05 * (386 - 0.1 * 386)
            2,
            "The chainable discount is applied since the minimum amount is reached",
        )

    def test_program_chainable_order_min_quantity_not_reached(self):
        order = self.empty_order
        self.env["sale.order.line"].create(
            {
                "product_id": self.largeCabinet.id,
        ...
        self.env["coupon.program"].create(
            {
                ...
                "sequence": 30,  # Comes after 10%
                "rule_minimum_amount": 350.00,
                "rule_minimum_amount_chained": True,
                "chainable": True,
            }
        )
        ...
        self.assertEqual(
            order.no_code_promo_program_ids.ids, [self.all_products_promo.id]
        )

    def test_program_chainable_order_min_quantity_reached(self):
        order = self.empty_order
        self.env["sale.order.line"].create(
            {
                "product_id": self.largeCabinet.id,
        ...
        self.assertEqual(
            order.no_code_promo_program_ids.ids,
            [self.all_products_promo.id, program.id],
        )
